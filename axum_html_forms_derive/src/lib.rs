...
pub fn derive(input: TokenStream) -> TokenStream {
    ...
    let tryfrom_fields = fields.iter().map(|f| {
        ...
        let ty = &f.ty;
        quote! {
            if let Some(v) = value.#ident.as_ref() {
                // TODO when this is generated by a proc macro, we need to maintain
                // a blacklist of  all field types that should never return the
                // invalid data back to the user (for example passwords)
                form.fields.#ident.value = value.#ident.clone();
                match v.parse::<#ty>() {
                    Ok(v) => #ident = Some(v),
                    Err(e) => {
                        form.fields.#ident.errors.push(String::from(e));
                    }
                }
            }
        }
